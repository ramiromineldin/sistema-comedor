...
public class RecurringDayDiscountStrategy implements PromotionStrategy {
    ...
    public void configure(Promotion promotion, PromotionCreateDTO request) {
        ...
        promotion.setRecurringDay(request.recurringDay());
        promotion.setStartDate(LocalDateTime.now());
        promotion.setEndDate(LocalDateTime.now().plusYears(1));
    }
    ...
    public void validate(PromotionCreateDTO request) {
        ...
        if (request.recurringDay() == null) {
            throw new IllegalArgumentException("El día de la semana es obligatorio para promociones recurrentes");
        }
    }
